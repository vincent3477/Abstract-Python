...
Original file is located at
    https://colab.research.google.com/drive/13v-mbk5-2kitALBpVLs_9HvnBNzwenv5
...
class SlidingWindowAveragerator(object):
    ...
    def std(self):
      avgNum = self.avg
      lenlist =len(self.values)
      temp=0
      for i in self.values:
        temp+=i**2

      return np.sqrt((temp / len(self.values) - avgNum * avgNum))
...
for _ in range(10):
    sa.add(8)
print("the average here is: " + str(sa.avg))
assert sa.avg == 8
assert sa.std == 0
...
class CleanData(object):
    ...
    def __init__(self, discount_factor):
        ...
        self.da = DiscountedAveragerator(self.discount_factor)
...
class MotionDetection(object):
    ...
    def __init__(self, num_sigmas=4., discount=0.96):
        ...
        """
        # YOUR CODE HERE
        self.da = DiscountedAveragerator(discount)
        self.numbersigmas = num_sigmas
    ...
    def detect_motion(self, img):
        ...
        average=self.da.avg
        std = self.da.std
        high = img < average-(self.numbersigmas*std)
        low = img > average+(self.numbersigmas*std)
        return np.logical_or(np.max(high,axis=2),np.max(low,axis=2))
