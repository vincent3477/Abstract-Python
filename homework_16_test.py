# -*- coding: utf-8 -*-
"""Homework_16_test.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1doEYQcBBsne2mvdbPy5hQEAoiUntAOVX
...
def decode_variable(p):
    ...
    if p > 988 or p < 100:
      return None
    if p%10 > 8 or p%10 < 0 or (p//10)%10 > 9 or (p//10)%10 < 0 or p//100>9 or p//100<1:
      return None
    return(p//100,(p//10)%10,p%10)
...
def every_cell_contains_at_least_one_digit():
    ...
    for i in range(9):
      for j in range(9):
        for x in range(1,10):
          list1.append(encode_variable(x,i,j))
    ...
    cellCount = 0
    lenList1 = len(list1)
    for y in range(lenList1):
      list2[cellCount].append(list1[y])
      count += 1
      if count == 9 and y != lenList1 - 1:
        list2.append([])
        count = 0
        cellCount += 1
    ...
    for i in range(9):
      for j in range(9):
        for x in range(1,10):
          for y in range(1,10):
            if y != x:
               list1.append([-encode_variable(x,i,j),-encode_variable(y,i,j)])
    ...
    for i in range(9):
      for j in range(9):
        for x in range(1,10):
          for r in range(9):
            if r != j:
              list1.append([-encode_variable(x,i,j),-encode_variable(x,i,r)])
    ...
    for i in range(9):
      for j in range(9):
        for x in range(1,10):
          for r in range(9):
            if r != i:
              list1.append([-encode_variable(x,i,j),-encode_variable(x,r,j)])
    ...
    for i in range(0,9):
      for j in range(0,9):
        ...
        for x in range(1,10):
          ...
          for x1 in range(i1,i1+3):
            for r in range(j1, j1+3):

              if x1 != i or r != j:
                list1.append([-encode_variable(x,x1,j),-encode_variable(x,i,r)])
...
def _board_to_SAT(self):
    ...
    for i in range(0,9):
      for j in range(0,9):
        if self.board[i][j] > 0:
          clauseList.append([encode_variable(self.board[i][j],i,j)])
    return clauseList
...
def solve(self, Solver):
    ...
    with Solver() as g:
      for i in self._to_SAT():
        g.add_clause(i)
      check_equal(g.solve(), True)
      missing = g.get_model()
      for m in missing:
        if m > 0 and decode_variable(m) is not None:
          iter1, iter2, iter3 = decode_variable(m)
          self.board[iter2, iter3] = iter1
